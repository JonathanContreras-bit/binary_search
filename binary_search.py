...
def count_repeats(xs, x):
    ...
    def low_i(ys):
        if ys:
            if (ys[-1] <= x) and (ys[0] >= x):
                if ys[0] == x:
                    return 0
# to avoid futile cycling
                mid = (len(ys) // 2) - 1
                if ys[mid] > x:
                    return mid + 1 + low_i(ys[mid + 1:])
                if ys[mid] <= x:
                    return low_i(ys[:mid + 1])


    def high_i(zs):
        if zs:
            if (zs[-1] <= x) and (zs[0] >= x):
                if zs[-1] == x:
                    return len(zs) - 1

                mid = len(zs) // 2
                if zs[mid] >= x:
                    return mid + high_i(zs[mid:])
                if zs[mid] < x:
                    return high_i(zs[:mid])


    try:
        return high_i(xs) - low_i(xs) + 1
    except TypeError:
        return 0
    def low_i(left, right):
        if left <= right:
            if xs[left] == x:
                return left
        ...
        return low_i(left, right)


    def high_i(left, right):
        if left <= right:
            if xs[right] == x:
                return right
        ...
        return high_i(left, right)

    return high_i(0, len(xs) - 1) - low_i(0, len(xs) - 1) + 1
    '''
    def low_i(left, right):
        if left < right:
            if xs[left] == x:
    ...
    if xs:
        hi_i = high_i(0, len(xs) - 1)
        lo_i = low_i(0, len(xs) - 1)
...
def argmin(f, lo, hi, epsilon=1e-3):
    ...
    mini = min(f(m1), f(m2), f(lo), f(hi))

    if mini == f(m1) or mini == f(lo):
        return argmin(f, lo, m2, epsilon)

    if mini == f(m2) or mini == f(hi):
